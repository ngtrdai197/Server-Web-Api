        ...
        if (err) {
            if (err.kind === 'ObjectId') {
                return res.status(404).send({
                    message: 'CategoryParent not found with id:' + req.params.id
                });
            }
            return res.status(500).send({
                message: err.message
...
exports.update = (req, res) => {
    CategoryParent.findByIdAndUpdate(req.params.id, req.body, {
        new: true
    }).exec((err, categoryParent) => {
        if (err) {
            if (err.kind === 'ObjectId') {
                return res.status(404).send({
                    message: 'CategoryParent not found with id:' + req.params.id
